...
\title{Beyond TritonRL: Triton Kernel Generation via Multi-Input Testing, Staged Evaluation, Adaptive Curriculum, and Calibrated Timing\\[0.8em]
...
The approach fine-tunes Qwen2.5-Coder-7B~\cite{qwen}, a state-of-the-art code generation model, on Triton kernel examples from the KernelBook dataset~\cite{kernelbook}. The training uses:
\begin{itemize}
...
\begin{itemize}
    \item \textbf{Source:} KernelBook dataset~\cite{kernelbook} (18,000 total kernel implementations)
...
The limited training scale restricts direct comparison to the baseline TritonRL system, which trained on significantly more data. Performance differences could be attributed to extensions, training data quantity (200 vs. 18K samples), hyperparameter choices, or extension interactions. Controlled experiments at comparable scale are needed for definitive attribution.
...
Testing all 16 extension configurations would isolate whether specific extensions help, hurt, or interact poorly.
...
Verification engineering took substantial development effort, comparable to the model training infrastructure.
...
With 4 binary extensions, establishing causal attribution needs all $2^4 = 16$ configurations tested.
...
This question remains open for RL code generation.
...
This work provides:
\begin{itemize}
    \item Complete implementation of four independent extensions
...
I would also like to thank friends and colleagues from the Catalyst Research Group at Carnegie Mellon University, for insightful discussions and technical feedback on verification infrastructure design and RL training protocols. Any remaining errors are my own.
...
\bibitem{kernelbook}
Paliskara, Sahan and Saroufim, Mark.
\emph{KernelBook}.
2025. \url{https://huggingface.co/datasets/GPUMODE/KernelBook}.

\bibitem{kernelbench}
Ouyang, Anne, Guo, Simon, Arora, Simran, Zhang, Alex L., Hu, William, Ré, Christopher, and Mirhoseini, Azalia.
\emph{KernelBench: Can LLMs Write Efficient GPU Kernels?}
arXiv preprint arXiv:2502.10517, 2025. \url{https://arxiv.org/abs/2502.10517}.
