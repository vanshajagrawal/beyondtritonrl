...
\affil{vmagrawa@andrew.cmu.edu}
\affil{Carnegie Mellon University}
\date{December 15, 2025}
\maketitle

\begin{abstract}
Machine learning system performance fundamentally depends on efficient GPU kernels. TritonRL uses reinforcement learning to generate Triton kernels from natural language descriptions, achieving 63\% correctness on basic operations (Level 1) but only 7\% on kernel fusion tasks (Level 2). We implemented four modular extensions to address specific weaknesses: multi-input testing to prevent overfitting, staged evaluation for computational efficiency, adaptive curriculum learning for progressive difficulty, and calibrated timing to reduce measurement noise. Due to computational and time constraints, we trained on 200 samples (1.1\% of the full dataset) with 2 RL fusion tasks over 20 minutes. Training completed successfully with SFT loss decreasing from 0.757 to 0.199 and RL final loss of 0.17. The limited training scale restricts comparison to the baseline system. We provide methodological insights on sample efficiency versus verification rigor tradeoffs and the challenges of evaluating multi-component RL systems at limited scale.
\end{abstract}
...
This project investigates whether modular extensions targeting specific weaknesses can improve upon the TritonRL baseline. We focus on four research questions:
...
We identify four specific weaknesses:
\begin{itemize}
...
We fine-tune Qwen2.5-Coder-7B~\cite{qwen}, a state-of-the-art code generation model, on Triton kernel examples from the KernelBook dataset~\cite{kernelbench}. We use:
\begin{itemize}
...
Following SFT, we apply reinforcement learning to optimize for both correctness and performance. The RL phase uses:
\begin{itemize}
...
5 & Calibrated timing & minutes & 1.0 \\
\bottomrule
\end{tabular}
...
We start with 10\% Level 2 tasks. As Level 1 accuracy improves toward 40\%, we linearly increase Level 2 proportion to 50\%. This progressive difficulty schedule allows the model to build strong foundations before tackling complex fusion tasks.

\textbf{Limitation:} With only 200 training samples and 20 minutes of training, we did not train long enough to observe curriculum effects.
...
We used AWS EC2 p4d.24xlarge spot instances with:
\begin{itemize}
...
Total training time: 20 minutes (5 minutes SFT + 15 minutes RL)
...
\begin{itemize}
    ...
    \end{itemize}
    \item \textbf{SFT:} All 200 samples, 1 epoch, 50 steps (5 minutes)
    \item \textbf{RL:} 2 Level 2 fusion tasks (15 minutes)
    \begin{itemize}
        \item Task 0: Fused GEMM + bias + ReLU
        \item Task 1: Fused Conv2d + BatchNorm + ReLU
    \end{itemize}
...
\begin{itemize}
    \item \textbf{SFT:} Loss decreased 0.757 → 0.199 (50 steps, 5 minutes)
    \item \textbf{RL:} Completed 2/2 fusion tasks (15 minutes)
...
\begin{itemize}
    ...
    \item \textbf{Time:} 20 minutes total vs. likely hours for baseline
\end{itemize}

The limited training scale restricts direct comparison to the baseline TritonRL system, which likely trained on significantly more data for longer duration. Performance differences could be attributed to extensions, training data quantity (200 vs. 18K samples), training time (20 min vs. hours), hyperparameter choices, or extension interactions. Controlled experiments at comparable scale would be required for definitive attribution.
...
This project had a fixed deadline. Training 200 samples with 2 RL tasks took 20 minutes. Scaling to 1,000+ samples and 10+ RL tasks would require multiple hours, which exceeded available time before the deadline.
...
We chose Option A based on the hypothesis that sample quality matters more than quantity for RL. Whether Option B's higher sample diversity outweighs the noise depends on the model's capacity to average over noisy signals.
...
Verification engineering time is comparable to model training time.
...
TritonRL achieves only 7\% correctness on kernel fusion tasks (Level 2), despite 63\% on basic operations (Level 1). We implemented four modular extensions targeting specific weaknesses: multi-input testing, staged evaluation, adaptive curriculum, and calibrated timing.

Training completed successfully: SFT loss decreased from 0.757 to 0.199 in 5 minutes, and RL training completed 2 fusion tasks in 15 minutes with final loss 0.17. We trained on 200 samples (1.1\% of the 18K-sample dataset) for 20 minutes total, compared to likely hours of training at full scale for the baseline.
...
\begin{itemize}
    ...
    \item Performance may be limited by extensions, data quantity, training time, or hyperparameters
\end{itemize}
...
I am grateful to the instructors and teaching staff for their guidance throughout this project. Special thanks to colleagues who provided technical feedback on verification infrastructure design and RL training protocols.
