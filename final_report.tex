...
% Use the following line for submission:
...
\textbf{Training Pipeline:} SFT Phase (Qwen2.5-7B + LoRA) $\rightarrow$ RL Phase (Best-of-N Sampling) $\rightarrow$ Extension Stack (Multi-Input, Staged Eval, Curriculum, Timing) $\rightarrow$ Reward Function [0-1]

\textbf{Data Flow:} KernelBook (18K samples) $\rightarrow$ 1K training subset $\rightarrow$ SFT $\rightarrow$ RL $\rightarrow$ Evaluation
...
\textbf{Training was initiated but encountered implementation bugs.} We successfully completed SFT training but evaluation failed due to code extraction issues. The following presents actual measurements and identified problems:

\textbf{Actual Training Metrics:}
...
\bottomrule
\end{tabular}
\caption{Actual training results from g5.2xlarge run}
\label{tab:results}
\end{table}

\textbf{Known Issues Encountered:}
\begin{enumerate}
\item \textbf{Code Extraction Bug:} Evaluation verifier failed to extract generated Triton code from model output, causing all kernels to be marked as invalid (0\% valid rate). The model generates text in chat format with markdown code blocks, but the extraction regex was too strict.

\item \textbf{Checkpoint Path Mismatch:} RL phase saved to \texttt{rl\_best\_of\_n/} instead of configured \texttt{rl\_final/}, causing evaluation to fail when loading the checkpoint.

\item \textbf{Instance Termination:} All training instances were terminated before bugs could be debugged and fixed, preventing rerun.
\end{enumerate}
...
\begin{itemize}
\item No valid experimental results on KernelBench Level 2 tasks
\item Cannot report improvements over TritonRL baseline
\item Extensions' impact on model accuracy unvalidated
\item No ablation studies conducted
\end{itemize}
...
\begin{itemize}
\item \textbf{No successful end-to-end evaluation.} Implementation bugs prevented measuring extension effectiveness.
\item \textbf{Component verification only.} Extensions tested in isolation (10-50 samples), not integrated system.
\item \textbf{No baseline comparison.} Cannot compare against TritonRL due to incomplete runs.
\end{itemize}
...
\item Unit-level verification of components
\item Honest analysis of implementation challenges
\item Lessons for future work on RL-based code generation
\end{itemize}
...
\textbf{Contributions:} (1) Complete implementations of four modular extensions, (2) architectural designs enabling ablation studies, (3) unit-level validation of each component, (4) honest analysis of end-to-end integration challenges in RL-based code generation systems.

\textbf{Limitations:} Implementation bugs in the evaluation pipeline prevented successful end-to-end training. SFT training completed (loss: 0.757$\rightarrow$0.199) but code extraction failures caused 0\% valid rate. This work represents implementation contributions and lessons learned rather than experimental validation of improvements.
...
\textbf{Code Repository:} \url{https://github.com/vanshajagrawal/tritonrl}
