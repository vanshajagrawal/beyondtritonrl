...
P. Tillet et al.,
``Triton: An intermediate language and compiler for tiled neural network computations,''
\textit{Proceedings of the ACM SIGPLAN International Conference on Programming Language Design and Implementation (PLDI)}, 2019.
...
% Additional appendices for completeness
\appendix
\section{Implementation Details}

\subsection{Hyperparameter Settings}

All experiments used the following hyperparameters unless otherwise specified:

\begin{itemize}
\item Learning rate: 1e-5 for SFT, 1e-6 for RL
\item Batch size: 16 for SFT, 8 for RL
\item Sequence length: 8192 tokens
\item Gradient accumulation: 4 steps
\item Mixed precision: BF16 training
...
\begin{itemize}
\item SFT phase: 4x A100 GPUs (40GB each)
\item RL phase: 2x H100 GPUs (96GB each)
\item Total compute: ~150 GPU-hours
\end{itemize}
...
\begin{enumerate}
\item \textbf{Validity}: Code compiles without syntax errors
\item \textbf{Compilation}: Code successfully compiles to Triton kernels
\item \textbf{Correctness}: Generated kernels produce correct outputs
\end{enumerate}
